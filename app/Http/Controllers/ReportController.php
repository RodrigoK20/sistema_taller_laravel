...
class ReportController extends Controller
{
    ...
    public function report_results(Request $request){
        ...
        return view('admin.report.reports_date', compact('sales', 'total','totaltaller')); 
            ...
            JOIN clients cl ON cl.id = s.client_id WHERE s.status= "VALID" AND s.sale_date BETWEEN :fi AND :ff GROUP BY s.id, s.sale_date, s.total, s.tax, sl.quantity, sl.price, sl.discount, sl.gain, p.name, u.name, p.sell_price, cl.name, pd.price ORDER BY s.id ASC', ['fi'=>$fi, 'ff'=>$ff]);
